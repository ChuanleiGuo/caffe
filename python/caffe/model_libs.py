...
def VGG19NetBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
        dilated=False, nopool=False, dropout=True, freeze_layers=[]):
    kwargs = {
            'param': [dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
            'weight_filler': dict(type='xavier'),
    ...
    if nopool:
        name = 'conv3_5'
        net[name] = L.Convolution(net.relu3_4, num_output=256, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool3'
        net[name] = L.Pooling(net.relu3_4, pool=P.Pooling.MAX, kernel_size=2, stride=2)
    ...
    net.conv4_3 = L.Convolution(net.relu4_2, num_output=512, pad=1, kernel_size=3, **kwargs)
    net.relu4_3 = L.ReLU(net.conv4_3, in_place=True)
    net.conv4_4 = L.Convolution(net.relu4_3, num_output=512, pad=1, kernel_size=3, **kwargs)
    net.relu4_4 = L.ReLU(net.conv4_4, in_place=True)

    if nopool:
        name = 'conv4_5'
        net[name] = L.Convolution(net.relu4_4, num_output=512, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool4'
        net[name] = L.Pooling(net.relu4_4, pool=P.Pooling.MAX, kernel_size=2, stride=2)
    ...
    net.conv5_3 = L.Convolution(net.relu5_2, num_output=512, pad=1, kernel_size=3, **kwargs)
    net.relu5_3 = L.ReLU(net.conv5_3, in_place=True)
    net.conv5_4 = L.Convolution(net.relu5_3, num_output=512, pad=1, kernel_size=3, **kwargs)
    net.relu5_4 = L.ReLU(net.conv5_4, in_place=True)

    if need_fc:
        if dilated:
            if nopool:
                name = 'conv5_5'
                net[name] = L.Convolution(net.relu5_4, num_output=512, pad=1, kernel_size=3, stride=1, **kwargs)
            else:
                name = 'pool5'
                net[name] = L.Pooling(net.relu5_4, pool=P.Pooling.MAX, pad=1, kernel_size=3, stride=1)
        else:
            if nopool:
                name = 'conv5_5'
                net[name] = L.Convolution(net.relu5_4, num_output=512, pad=1, kernel_size=3, stride=2, **kwargs)
            else:
                name = 'pool5'
                net[name] = L.Pooling(net.relu5_4, pool=P.Pooling.MAX, pad=1,kernel_size=3, stride=1)
